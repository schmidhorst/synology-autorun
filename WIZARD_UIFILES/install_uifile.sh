...
if [ ! -f "$configFilePathName" ]; then
  logInfo 7 "No old configuration file '$configFilePathName' from a previous installation before V1.10 found, using initial config"
...
msg=""
for f1 in $fields; do # get the item value now either from latest configuration (if it was not deleted at uninstall) or the default value from the intial config file
...
fingerprints="${SCRIPTPATHwiz%%/@*}/@appdata/${SYNOPKG_PKGNAME}/FINGERPRINTS" # e.g. /volume1/@appdata/${SYNOPKG_PKGNAME}/FINGERPRINTS  
...
if [[ "$ENTRY_COUNT" -eq "0" ]]; then
  ...
  if [[ -f "$scriptpathParent/package/ui/texts/$lng/lang.txt" ]]; then
    ...
    eval "$(grep "notAccepted=" "$scriptpathParent/package/ui/texts/$lng/lang.txt")" # "
  fi
  if [[ -z "$HASH_DELETION" ]]; then
    HASH_DELETION="<b>Previous registered hashes are all deleted now.</b> "
  fi
  if [[ -z "$notAccepted" ]]; then
...
### die automatische Übersetzung einfügen!
